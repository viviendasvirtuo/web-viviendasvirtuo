...
export const metadata: Metadata = {
  ...
  description: 'Gestión profesional de apartamentos vacacionales en Barcelona. Maximiza la rentabilidad de tu propiedad con gestión completa.',
};

const VACACIONAL_ORANGE = '#e07000';
const VACACIONAL_ORANGE_DARK = '#c45e00';
const VACACIONAL_ACCENT = '#ffd580';
      ...
      <main>
        ...
        <section style={{ position: 'relative', paddingTop: '140px', paddingBottom: '80px', overflow: 'hidden', minHeight: '520px' }}>
          <Image
            ...
            priority
          />
          <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, ${VACACIONAL_ORANGE_DARK}e8 0%, ${VACACIONAL_ORANGE}cc 100%)` }} />
          ...
          <div className="container" style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.2rem, 4.5vw, 3.5rem)', fontWeight: 900, color: 'white', lineHeight: 1.1, marginBottom: '20px', maxWidth: '680px' }}>
              Rentabilidad máxima con <span style={{ color: VACACIONAL_ACCENT }}>gestión cero</span>
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.78)', fontSize: 'var(--text-lg)', maxWidth: '540px', lineHeight: 1.7, marginBottom: '40px' }}>
              Gestionamos tu apartamento en Airbnb y plataformas similares. Tú cobras, nosotros nos encargamos de todo.
            </p>
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <Link href="/propietarios" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'white', color: VACACIONAL_ORANGE_DARK, fontWeight: 700, padding: '14px 28px', borderRadius: 'var(--radius-lg)', textDecoration: 'none', fontSize: 'var(--text-base)' }}>Quiero rentabilizar mi piso</Link>
        ...
        <section style={{ padding: 'clamp(60px,8vw,100px) 0', background: 'var(--color-bg)' }}>
          <div className="container">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(480px, 100%), 1fr))', gap: 'clamp(40px, 6vw, 80px)', alignItems: 'center' }}>
              <div>
                <p style={{ color: VACACIONAL_ORANGE_DARK, fontWeight: 700, fontSize: 'var(--text-sm)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '12px' }}>Gestión vacacional Virtuo</p>
                <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.6rem,2.5vw,2.2rem)', fontWeight: 800, color: 'var(--color-text)', lineHeight: 1.2, marginBottom: '20px' }}>Tu apartamento trabaja para ti</h2>
                <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.75, marginBottom: '24px', fontSize: 'var(--text-base)' }}>Gestionamos apartamentos completos para alquiler vacacional. Nos encargamos de la publicación, reservas, check-in, limpieza y atención al huésped. Tú solo ves los ingresos.</p>
                <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.75, marginBottom: '32px', fontSize: 'var(--text-base)' }}>Aplicamos precios dinámicos para maximizar la ocupación y la rentabilidad en cada temporada.</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
                  {[
                    'Publicación en Airbnb, Booking y plataformas principales',
                    'Gestión completa de reservas y calendario',
                    'Check-in / check-out y atención al huésped 24h',
                    'Limpieza y lavandería entre estancias',
                    'Precios dinámicos optimizados por temporada',
                ...
                <Link href="/propietarios" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: VACACIONAL_ORANGE, color: 'white', fontWeight: 700, padding: '14px 28px', borderRadius: 'var(--radius-lg)', textDecoration: 'none', fontSize: 'var(--text-base)' }}>Soy propietario →</Link>
              </div>
              <div style={{ position: 'relative', borderRadius: 'var(--radius-xl)', overflow: 'hidden', aspectRatio: '4/3', boxShadow: `0 20px 60px ${VACACIONAL_ORANGE}2a` }}>
                <Image
                  src="https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=900&q=80&auto=format&fit=crop"
                ...
                <div style={{ position: 'absolute', bottom: '20px', left: '20px', background: 'rgba(255,255,255,0.95)', borderRadius: 'var(--radius-lg)', padding: '12px 16px', boxShadow: '0 4px 20px rgba(0,0,0,0.12)' }}>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-lg)', fontWeight: 900, color: VACACIONAL_ORANGE_DARK, lineHeight: 1 }}>+60%</div>
        ...
        <section style={{ padding: 'clamp(60px,8vw,100px) 0', background: '#f0f4fa' }}>
          <div className="container">
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.6rem,2.5vw,2.2rem)', fontWeight: 800, color: 'var(--color-text)' }}>¿Para qué propietarios es ideal?</h2>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px,1fr))', gap: '20px' }}>
              {[
                { tipo: 'Propietarios con licencia turística', desc: 'Aprovecha al máximo tu licencia con gestión profesional.' },
                { tipo: 'Pisos en zonas turísticas', desc: 'Barrios céntricos y costeros con alta demanda vacacional.' },
                { tipo: 'Segunda residencia', desc: 'Genera ingresos cuando no usas tu apartamento.' },
                { tipo: 'Inversores turísticos', desc: 'Maximiza el retorno de tu inversión en Barcelona.' },
              ].map((p, i) => (
        ...
        <section style={{ padding: 'clamp(60px,8vw,100px) 0', background: `linear-gradient(135deg, ${VACACIONAL_ORANGE_DARK}, ${VACACIONAL_ORANGE})`, textAlign: 'center' }}>
          <div className="container">
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.6rem,3vw,2.2rem)', fontWeight: 800, color: 'white', marginBottom: '16px' }}>¿Quieres rentabilizar tu apartamento?</h2>
            <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 'var(--text-lg)', maxWidth: '460px', margin: '0 auto 36px', lineHeight: 1.7 }}>Cuéntanos tu situación y calculamos la rentabilidad estimada sin compromiso.</p>
            <Link href="/propietarios" style={{ display: 'inline-flex', alignItems: 'center', background: 'white', color: VACACIONAL_ORANGE_DARK, fontWeight: 700, padding: '14px 32px', borderRadius: 'var(--radius-lg)', textDecoration: 'none', fontSize: 'var(--text-base)' }}>Hablar con un gestor</Link>
          </div>
        </section>
      </main>
      <Footer />
