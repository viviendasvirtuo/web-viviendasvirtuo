...
export default function HabitacionesDestacadas({ sistema, sistemas, mostrarTitulo = true }: Props) {
  ...
  // Con mostrarTitulo=false el componente devuelve SOLO las tarjetas (sin section, sin grid)
  // El grid y el layout lo gestiona quien llama al componente
  if (!mostrarTitulo) {
    return (
      <>
        {lista.map((hab) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px', flex: 1 }}>
                ...
                <p style={{ fontSize: '0.8rem', color: '#9aa5b4', margin: 0 }}>{hab.detalles.join(' · ')}</p>
                <a
                  href={hab.tallyUrl}
                  target="_blank"
  ...
  // Con mostrarTitulo=true el componente gestiona su propio layout completo
  return (
    <section style={{ background: '#f8f9fc', padding: '80px 0' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: '56px', maxWidth: '700px', margin: '0 auto 56px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 700, color: '#0f2d5e', marginBottom: '16px', lineHeight: 1.2 }}>
            {sistemaEfectivo === 'vacacional' ? 'Apartamentos disponibles en Barcelona' : 'Encuentra tu habitación en Barcelona'}
          </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          <HabitacionesDestacadas sistema={sistema} sistemas={sistemas} mostrarTitulo={false} />
        </div>
